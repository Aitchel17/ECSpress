function [div_cells, anchor_xy] = piv_divergence(flow, sectormap, n_labels)
...
%      n_labels   from info.n_labels; [] = read it off the map
...
if ~isequal(size(lab), [H, W])
    error('piv_divergence:mapSize', 'sectormap must be %dx%d.', H, W);
end
...
for k = 1:n_labels
    ...
    xi = X(sel);  yi = Y(sel);
    % 1.1. plane_divergence returns ok = false on a rank-deficient spread
    [dval, ok] = plane_divergence(xi, yi, u(sel), v(sel));
    if ~ok; continue; end
    div_cells(k)    = dval;
