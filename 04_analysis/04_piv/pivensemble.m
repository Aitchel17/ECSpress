...
%% [DEVIATION] Derive video_frame_selection from imgstack dimensions.
% Simulates the original video_frame_selection as consecutive indices 1:N.
...
tic
	...
	image1 = imgstack(:,:,video_frame_selection(sli_idx));
	image2 = imgstack(:,:,video_frame_selection(sli_idx+1));
	...
	mask_inpt = converted_mask{floor((sli_idx+1)/2),1};
	if numel(roi_inpt)>0
		xroi = roi_inpt(1);
		yroi = roi_inpt(2);
		widthroi = roi_inpt(3);
...
%correlation_map=[];
	...
	for jmask = miniy:step:maxiy %vertical loop
		...
		for imask = minix:step:maxix % horizontal loop
			...
			%fehlerzeile:
			if average_mask_roi(round(jmask+interrogationarea/2),round(imask+interrogationarea/2)) >= num_slice/2
