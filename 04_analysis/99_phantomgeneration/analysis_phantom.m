...
% Path setup
addpath(fullfile(pwd, 'functions'));                    % where dirs_ecspath lives
...
%% Make phantom and overwrite original data (to show perferct symetrical dilation vs asymmetric dilation) but retain original dimension
%% Phantom parameters — match original image dimensions & frame count
[img_rows, img_cols, n_frames] = size(twophoton_processed.ch1);
cx = img_cols / 2;     % center x (always fixed)
cy = img_rows / 2;     % center y (always fixed)

% Radius modulation: oscillates between r_min and r_max (2 full cycles)
r_base   = min(img_rows, img_cols) * 0.10;   % baseline radius (20% of shorter dim)
r_amp    = r_base * 0.25;                     % ±25% amplitude
r_series = r_base + r_amp * sin(linspace(0, 4*pi, n_frames));

% Coordinate grids (computed once)
[XX, YY] = meshgrid(1:img_cols, 1:img_rows);

% --- 1. Symmetric phantom: center fixed, radius expands equally in all directions ---
phantom_symmetric = zeros(img_rows, img_cols, n_frames, 'like', twophoton_processed.ch1);
for f = 1:n_frames
    dist = sqrt((XX - cx).^2 + (YY - cy).^2);
    phantom_symmetric(:,:,f) = cast(dist <= r_series(f), 'like', twophoton_processed.ch1);
end

% --- 2. Asymmetric phantom: left edge pinned, right edge dilates ---
% Left wall stays at (cx - r_base); center shifts right as radius grows
phantom_asymmetric = zeros(img_rows, img_cols, n_frames, 'like', twophoton_processed.ch1);
x_left_fixed = cx - r_base;
for f = 1:n_frames
    r = r_series(f);
    cx_asym = x_left_fixed + r;   % shift center so left edge stays fixed
    dist = sqrt((XX - cx_asym).^2 + (YY - cy).^2);
    phantom_asymmetric(:,:,f) = cast(dist <= r, 'like', twophoton_processed.ch1);
end

% Wrap into twophoton_processed copies for downstream pipeline use
twophoton_processed_sym      = twophoton_processed;
twophoton_processed_sym.ch1  = phantom_symmetric;

twophoton_processed_asym     = twophoton_processed;
twophoton_processed_asym.ch1 = phantom_asymmetric;

%% 4.1 FWHM Analysis
% 4.1.1 FWHM analysis - ROI Setupw
session.roilist.addormodifyroi(twophoton_processed_asym.ch1 ,'pax','line');
...
session.roilist.addormodifyroi(twophoton_processed_asym.ch1,'radon','rectangle');
%%
session.radon_analysis = analysis_radon(twophoton_processed_sym, session.roilist, 'ch1');
...
cla
subplot(2,1,1)
imagesc(session.radon_analysis.radon_result.idx_downloc+session.radon_analysis.radon_result.idx_uploc)
ylabel("center change")
subplot(2,1,2)
imagesc(session.radon_analysis.radon_result.idx_downloc-session.radon_analysis.radon_result.idx_uploc)
