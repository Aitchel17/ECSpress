function idx_dataset = clean_idxoutlier(idx_dataset, overwrite, medfilt_size, noise_thr)
% CLEAN_OUTLIER Cleans outliers from index dataset by comparing with median filtered data.
    ...
    for idx_name = unprocessed_names
        if isempty(idx_name)
            ...
            ref_median = medfilt1(idx_data,medfilt_size);
            diff_data = abs(idx_data - ref_median);
            above_noise = diff_data > noise_thr;
